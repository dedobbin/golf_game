...
void parseBehavior(nlohmann::json jBehavior, Entity* owner)
{
	...
	if (owner->type == entityType::ITEM){
		...
		behavior = new MovingPlatformBehavior(owner, endPos);
		
		if (!jBehavior["potentional_acc"].is_null()){
			((MovingPlatformBehavior*)behavior)->potentionalAcc = jBehavior["potentional_acc"];
		}
	} else {
